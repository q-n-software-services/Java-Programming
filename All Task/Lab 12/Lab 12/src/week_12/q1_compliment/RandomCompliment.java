...
public class RandomCompliment {
    ...
    public static String getRandomCompliment() {
        ...
        */
        ...
        HttpResponse<JsonNode> response = Unirest.get("https://random-compliment.azurewebsites.net/random")
                .asJson();

        // Check the response status
        if (response.isSuccess()) {
            // Get the compliment from the response body
            String compliment = response.getBody().getObject().getString("text");
            System.out.println(compliment);
        } else {
            // Handle the request failure
            System.out.println("Failed to retrieve a random compliment");
        }
...
public class Compliment {
    ...
    // Make the request to the API
    HttpResponse<Compliment> response = Unirest.get("https://random-compliment.azurewebsites.net/random")
            .asObject(Compliment.class);

    // Check the response status
    if(response.isSuccess()){
        // Get the compliment from the response body
        Compliment compliment = response.getBody();
        String complimentText = compliment.getText();
        System.out.println(complimentText);
    } else {
        // Handle the request failure
        System.out.println("Failed to retrieve a random compliment");
    }
