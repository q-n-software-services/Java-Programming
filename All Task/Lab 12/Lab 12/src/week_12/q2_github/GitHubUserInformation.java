...
//        return null;  // TODO delete and replace with your code
        ...
        // Make the request to the GitHub API
        HttpResponse<GitHubRepository[]> response = Unirest.get(url)
                .header("Accept", "application/vnd.github.v3+json")
                .asObject(GitHubRepository[].class);

        // Check the response status
        if (response.isSuccess()) {
            // Get the repositories from the response body
            GitHubRepository[] repositories = response.getBody();
            return repositories;
        } else {
            // Handle the request failure
            System.out.println("Failed to retrieve repositories from GitHub");
            return null;
        }
    }
